...
import com.xuanyuetech.tocoach.util.StickerUtil;
...
public class StickerTextView extends PLTextView {
    ...
    private void init() {
        ...
        if (mControlDrawable == null) {
            mControlDrawable = getContext().getResources().getDrawable(R.drawable.ic_rotation_sticker);
        }
        ...
        if (mEditDrawable == null) {
            mEditDrawable = getContext().getResources().getDrawable(R.drawable.ic_edit_sticker);
        }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mStatus == STATUS_ROTATE_ZOOM) {
                    ...
                    //当前的点到 view 中心的距离
                    float curPointToViewCenterDistance = StickerUtil.getTwoPointsDistance(mCenterPoint, mCurMovePointF);
                    //之前的点到 view 中心的距离
                    float prePointToCenterDistance = StickerUtil.getTwoPointsDistance(mCenterPoint, mPreMovePointF);
                    //计算缩放幅度
                    ...
                    double a = StickerUtil.getTwoPointsDistance(mCenterPoint, mPreMovePointF);
                    double b = StickerUtil.getTwoPointsDistance(mPreMovePointF, mCurMovePointF);
                    double c = StickerUtil.getTwoPointsDistance(mCenterPoint, mCurMovePointF);
                    double cosb = (a * a + c * c - b * b) / (2 * a * c);
                    if (cosb >= 1) {
                        cosb = 1f;
                    }
                    double radian = Math.acos(cosb);
                    float newDegree = (float) StickerUtil.radianToDegree(radian);
    ...
    private void computeRect(int left, int top, int right, int bottom, float degree) {
        ...
        //计算 X 坐标最大的值和最小的值
        int maxCoordinateX = StickerUtil.getMaxValue(mLTPoint.x, mRTPoint.x, mRBPoint.x, mLBPoint.x);
        int minCoordinateX = StickerUtil.getMinValue(mLTPoint.x, mRTPoint.x, mRBPoint.x, mLBPoint.x);

        mViewWidth = maxCoordinateX - minCoordinateX;

        //计算 Y 坐标最大的值和最小的值
        int maxCoordinateY = StickerUtil.getMaxValue(mLTPoint.y, mRTPoint.y, mRBPoint.y, mLBPoint.y);
        int minCoordinateY = StickerUtil.getMinValue(mLTPoint.y, mRTPoint.y, mRBPoint.y, mLBPoint.y);
    ...
    public static Point obtainRotationPoint(Point center, Point source, float degree) {
        ...
        //弧度调整
        resultRadian = originRadian + StickerUtil.degreeToRadian(degree);
    ...
    private int judgeStatus(float x, float y) {
        ...
        //点击的点到控制旋转，缩放点的距离
        float disToControl = StickerUtil.getTwoPointsDistance(touchPoint, new PointF(mControlPoint));
        ...
        float disToEdit = StickerUtil.getTwoPointsDistance(touchPoint, new PointF(mEditPoint));
        if (disToEdit < Math.min(mEditDrawableWidth / 2, mEditDrawableHeight / 2)) {
            return STATUS_EDIT;
        }

        float disToDelete = StickerUtil.getTwoPointsDistance(touchPoint, new PointF(mDeletePoint));
